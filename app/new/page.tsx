...
export default function NewPostPage() {
  ...
  const submit = async (status: 'draft' | 'published') => {
    ...
    try {
      ...
      setTimeout(() => router.push(`/articles/${post.slug}`), 500)
    } catch (err) {
      let msg = err instanceof Error ? err.message : '保存失败'
      if (msg.includes('401') || msg.includes('登录')) {
        msg = '登录已过期，请重新登录'
        setTimeout(() => router.replace('/login?next=/new'), 800)
      }
      setError(msg)
  ...
  return (
    <section className="mx-auto max-w-3xl px-6 py-16 lg:px-8 lg:py-24">
      <p className="font-mono text-xs tracking-[0.2em] text-primary">新建文章</p>
      <h1 className="mt-3 text-3xl tracking-tight sm:text-4xl">写一篇文章</h1>
      <p className="mt-4 text-sm text-muted-foreground">
        写完点发布；没写完也可以存草稿，草稿只有管理员可见。
      </p>
